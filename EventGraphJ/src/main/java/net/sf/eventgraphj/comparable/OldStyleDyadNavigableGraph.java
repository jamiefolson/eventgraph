 ...
 * @param <E>
 */
public abstract class OldStyleDyadNavigableGraph<K extends Comparable<K>, V, E> extends BaseNavigableGraph<K, V, E>
	...
	public static class DyadNavigableSubGraph<K extends Comparable<K>, V, E> extends
	        OldStyleDyadNavigableGraph<K, V, E> {
		protected final OldStyleDyadNavigableGraph<K, V, E> parent;
		final K start, stop;

		public DyadNavigableSubGraph(OldStyleDyadNavigableGraph<K, V, E> parent, K start, K stop) {
			...
			for (DyadEdgeMap<K, V, E> edge : parent.mapGraph.getEdges()) {
				...
				this.mapGraph.addEdge(new DyadEdgeMap<K, V, E>(edge.from, edge.to, map), endpoints, edgetype);
			}
		}
	...
	@Inject
	public OldStyleDyadNavigableGraph(MapProvider mapProvider, GraphProvider graphProvider) {
		super();
		this.mapProvider = mapProvider;
		this.graphProvider = graphProvider;
	}

	protected OldStyleDyadNavigableGraph(MapProvider mapProvider, GraphProvider graphProvider, K lowerBound,
	        K upperBound) {
	...
	public boolean containsEdge(EdgeEntry<K, V, E> edge, K start, K stop) {
	...
	public EdgeEntry<K, V, E> findEdge(V v1, V v2, K start, K stop) {
		Collection<DyadEdgeMap<K, V, E>> maps = this.mapGraph.findEdgeSet(v1, v2);
		if (maps != null) {
			for (DyadEdgeMap<K, V, E> map : maps) {
				SortedMap<K, EdgeEntry<K, V, E>> submap = (start != null && stop != null) ? map.map.subMap(start, stop)
				        : map.map;
				if (submap.size() > 0) {
	...
	public Collection<EdgeEntry<K, V, E>> findEdgeSet(V v1, V v2, K start, K stop) {
		...
		if (maps != null) {
			for (DyadEdgeMap<K, V, E> map : maps) {
				...
				Collection<EdgeEntry<K, V, E>> vals = (start != null && stop != null) ? map.map.subMap(start, stop)
	...
	public int getEdgeCount(EdgeType edge_type, K start, K stop) {
		return this.getEdges(edge_type, start, stop).size();
	}
	...
	public Collection<EdgeEntry<K, V, E>> getEdges(EdgeType edge_type, K start, K stop) {
	...
	public Collection<EdgeEntry<K, V, E>> getEdges(K start, K stop) {
		Collection<EdgeEntry<K, V, E>> edges = new ArrayList<EdgeEntry<K, V, E>>();
	...
	public Collection<EdgeEntry<K, V, E>> getInEdges(V vertex, K start, K stop) {
		Collection<EdgeEntry<K, V, E>> neighbors = new ArrayList<EdgeEntry<K, V, E>>();
	...
	public int getNeighborCount(V vertex, K start, K stop) {
		return this.getNeighbors(vertex, start, stop).size();
	}
	...
	public Collection<V> getNeighbors(V vertex, K start, K stop) {
		...
		if (maps != null) {
			for (DyadEdgeMap<K, V, E> map : maps) {
				...
				for (Entry<K, EdgeEntry<K, V, E>> entry : (start != null && stop != null) ? map.map.subMap(start, stop)
	...
	public Collection<EdgeEntry<K, V, E>> getOutEdges(V vertex, K start, K stop) {
		Collection<EdgeEntry<K, V, E>> neighbors = new ArrayList<EdgeEntry<K, V, E>>();
	...
	public Collection<V> getPredecessors(V vertex, K start, K stop) {
		...
		if (maps != null) {
			for (DyadEdgeMap<K, V, E> map : maps) {
				for (EdgeEntry<K, V, E> edge : (start != null && stop != null) ? map.map.subMap(start, stop).values()
				        : map.map.values()) {
	...
	public int inDegree(V vertex, K start, K stop) {
		return this.getInEdges(vertex, start, stop).size();
	}
	...
	public int outDegree(V vertex, K start, K stop) {
		return this.getOutEdges(vertex, start, stop).size();
	}
	...
	public NavigableGraph<K, V, E> subNetwork(K start, K stop) {
		return new DyadNavigableSubGraph(this, start, stop);
	}

	@Override
	public NavigableGraph<K, V, E> tailNetwork(K start) {
		return new DyadNavigableSubGraph(this, start, null);
	}

	@Override
	public NavigableGraph<K, V, E> headNetwork(K stop) {
		return new DyadNavigableSubGraph(this, null, stop);
	}
}
